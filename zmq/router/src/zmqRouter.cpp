...
worker_task(void *args)
{
    ...
    while (1) {
		...
		std::cout << "[worker]: bose request: " << workload << std::endl;
        if ("Fired!" == workload) {
...
int main() {
    ...
    while (1) {
		...
		std::cout << "[Boss]: worker id: " << identity << std::endl;
        {
            std::string envelope = s_recv(broker);     //  Envelope delimiter
			std::cout << "[Boss]: woker Envelope: " << envelope << std::endl;

            std::string response = s_recv(broker);     //  Response from worker
			
			std::cout << "[Boss]: worker response: " << response << std::endl;
        }
